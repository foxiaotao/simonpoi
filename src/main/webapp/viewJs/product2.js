...
$(function(){
	ctx = $("#ctxHiidenId").val();
	 ...
	 $('#userTable').datagrid({  
         ...
         //url:$("#ctxHiddenId").val()+"Product/selectAll.do", //数据来源  
         url: ctx+"/Product/selectAll.do", //数据来源  
         sortName: 'product.id', //排序的列  
         sortOrder: 'desc', //倒序  
         ...
         columns:[[  
             ...
             {field:'product.id',title:'序号',width:20,sortable:true,  
                 formatter:function(value,row,index){return row.product.name;} //需要formatter一下才能显示正确的数据  
             },  
             {field:'product.product',title:'名称',width:20,sortable:true,  
                 formatter:function(value,row,index){return row.product.age;}  
             },  
             {field:'product.unit',title:'价格',width:30,sortable:true,  
                 formatter:function(value,row,index){return row.product.birthday;}  
             },  
             {field:'product.attr',title:'属性',width:30,sortable:true,  
                 formatter:function(value,row,index){return row.product.birthday;}  
             },  
             {field:'product.status',title:'状态',width:30,sortable:true,  
                 formatter:function(value,row,index){return row.product.birthday;}  
             },  
//             {field:'user.deptId',title:'属性',width:30,sortable:true,  
